...
let encodeNSA 
    ...
    let transitionFormula = 
        ...
        |> fun x -> 
            ...
            Forall ([(timeVar, timeSort); (timeVarNext, timeSort);], 
                Implies(
                    PredicateApp(succPredicate, [Variable timeVar; Variable timeVarNext]),
                    x
                )
            )
    ...
    let timeFormula = 
        Forall([timeVar, timeSort], Exists([timeVarNext, timeSort], PredicateApp(succPredicate, [Variable timeVar; Variable timeVarNext])))

    let finalFormula = And[timeFormula; specificFormula]
...
let computeEncodingSmtLib (config : Configuration) (hyperltl : HyperLTL<string>) = 
    ...
    let finalFormula = encodeNSA nsa hyperltl.QuantifierPrefix traceSort timeSort initTimeConstant succPredicate predicatesForApsMap predicatesForStatesMap
...
let computeEncodingTPTP (config : Configuration) (hyperltl : HyperLTL<string>) = 
...

    {
        FirstOrderTPTPInstance.Formulas = 
            [initTimeSort; existsTrace; finalFormula]
    }
