...
let run args =    
    ...
    let path = 
        cmdArgs.InputFile
        |> Option.defaultWith (fun _ -> raise <| HyperSatException "No input file given")

    let c = 
        try File.ReadAllText path with
        | _ -> raise <| HyperSatException $"Could not open {path}"
    ...
    | SMTLIB -> 
        ...
        config.Logger.LogN $"Constructed FOL encoding in %i{sw.ElapsedMilliseconds} ms (%.4f{double(sw.ElapsedMilliseconds) / 1000.0} s)"
        ...
        | Some solver -> 
            ...
            config.Logger.LogN $"Solved encoding in %i{sw.ElapsedMilliseconds} ms (%.4f{double(sw.ElapsedMilliseconds) / 1000.0} s)"
