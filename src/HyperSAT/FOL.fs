    ...
    let rec printSmtLib (varStringer : 'T -> string) (t : FirstOrderTerm<'T>) = 
        ...
        | FunctionApp (f, args) ->
            if List.isEmpty args then 
                ...
                "(" + f + " " + (args |> List.map (printSmtLib varStringer) |> String.concat " ") + ")"
...
module FirstOrderFormula = 
    let rec printSmtLib (varStringer : 'T -> string) (f : FirstOrderFormula<'T>) =
        ...
        | And args -> 
            ...
            | _ -> "(and " + (args |> List.map (printSmtLib varStringer) |> String.concat " ") + ")"
        | Or args -> 
            match args with 
            | [] -> "false"
            | [x] -> printSmtLib varStringer x
        ...
        | PredicateApp (p, args) -> "(" + p + " " + (args |> List.map (FirstOrderTerm.printSmtLib varStringer) |> String.concat " ") + ")"
        | Forall (prefix, f1) -> 
            let prefixString = prefix |> List.map (fun (x, s) -> "(" + varStringer x + " " + s + ")") |> String.concat " "
    ...
    let rec printTPTP (varStringer : 'T -> string) (f : FirstOrderFormula<'T>) =
        ...
        | Or args -> 
            ...
            | _ -> 
                ...
                |> List.map (printTPTP varStringer)
                |> List.reduce (fun a b -> "(" + a + ")" + " | " + "(" + b + ")")
        | Implies (f1, f2) -> "(" + printTPTP varStringer f1 + ")" + " => " + "(" + printTPTP varStringer f2 + ")"
        | Neg (f1) -> "~ (" + printTPTP varStringer f1 + ")"
        | PredicateApp (p, args) -> p + "(" + (args |> List.map (FirstOrderTerm.printTPTP varStringer) |> String.concat ",") + ")"
        | Forall (prefix, f1) -> 
            let prefixString = 
                prefix 
                |> List.map (fun (x, _) -> varStringer x)
                |> String.concat ","
            ...
            "! [" + prefixString + "] : (" + sortsString + " => " + printTPTP varStringer f1 + ")"
        | Exists (prefix, f1) -> 
            let prefixString = 
                prefix 
                |> List.map (fun (x, _) -> varStringer x)
...
module FirstOrderSmtLibInstance = 
    let print (varStringer : 'T -> string) (p : FirstOrderSmtLibInstance<'T>) = 
        ...
        let functionString = 
            p.FunctionSymbols
            |> List.map (fun (f, args, r) -> 
                "(declare-fun " + f + " (" + (args |> String.concat " ") + ") " + r +  ")"
                )
            |> String.concat "\n"
        ...
        let formulasString = 
            p.Formulas
            |> List.map (fun x -> 
                "(assert " + FirstOrderFormula.printSmtLib varStringer x + ")"
                )
            |> String.concat "\n"
