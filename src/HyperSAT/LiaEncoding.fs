...
let computeEncodingSmtLib (config : Configuration) (hyperltl : HyperLTL<string>) = 
    ...
    let nba =
        match FsOmegaLib.Operations.LTLConversion.convertLTLtoNBA false config.SolverConfig.MainPath config.SolverConfig.Ltl2tgbaPath hyperltl.LTLMatrix with 
        | Success x -> x
        | Fail err -> 
    ...
    let transitionFormula = 
        ...
        |> And
        |> fun x -> 
            Forall ([(timeVar, "Int")], x)
    
    let acceptingFormula = 
        Forall ([timeVar, "Int"], 
            Exists ([timeVarNext, "Int"], 
                [
                    ...
                    |> Seq.map (fun q -> 
                        Neg(
                            PredicateApp(
                                predicatesForStatesMap.[q], 
                                (Variable timeVarNext) :: (traceVariables |> List.map (fun pi -> Variable variablesForTraceVariablesMap.[pi]))
                            )
                        )
                    )
                    |> Seq.toList
